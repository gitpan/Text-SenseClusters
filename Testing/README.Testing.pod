...
using a script normal-op.sh(found in each sub-dir). Similarly, error-op.sh 
scripts (in sub-dirs) test program behavior under error conditions.

All test scripts in all the program directories can be run with a single 
command testall.sh. (subtestall.sh is internally called by testall.sh and 
shouldn't be run independantly)
