...
=head2 Version 0.97 ToDo

=over

=item 1. Improve order 1 efficiency. Rather than matching each context 
...
=item 2. Provide simple tools that allow a user to visualize more 
...
=item 3. Make discriminate.pl more modular in its organization, 
possibly through the use of subroutines. 

=item 4. Fix --showargs option on discriminate.pl. Has not been working
for many versions now. 

=item 5. Check to see if checking return values from vcluster and scluster 
in discriminate.pl is really accomplishing anything. Do they return error  
codes and success codes reliably? Any chance for false positive or false 
negative?

=item 6. Replace default stoplist with a program that automatically 
generates stoplists from a given corpus. 

=item 7. Add version information to SenseClusters web interface, including 
version of SenseClusters and modules it is using.

=item 8. There are various small utility programs whose return codes are
...
=item 9. Reduce the number of regular expressions used in the regex file
provided to order2vec.pl for feature identification during LSA style context
clustering. This is required if we adhere to
the nsp2regex.pl approach for feature identification. Right now regexes are 
generated from training data based on all the features found in the training 
data. These are given as input to order1vec.pl to identify features from
...
=item 10. Wherever possible and appropriate, add the error checks 
from disciminate.pl to the actual programs that require 
that error check.
For example: Check for 0 zero features in order1vec.pl, 
wordvec.pl and order2vec.pl

=item 11. Check why the criterion function values as different across
...
=item 12. The idea of global training data is to have one large file
...
=item 13. The gap statistic generates expected values for randomly
...
SenseClusters' Home Page: http://senseclusters.sourceforge.net

=head1 COPYRIGHT

Copyright (C) 2005-2006 Ted Pedersen, Amruta Purandare and Anagha Kulkarni
